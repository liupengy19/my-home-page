...
export default async function OGImage(
  ...
  const requestedPageId = Array.isArray(req.query.id)
    ? req.query.id[0]
    : req.query.id
  const pageId = parsePageId(requestedPageId || libConfig.rootNotionPageId)
  if (!pageId) {
  ...
  for (const [name, value] of imageResponse.headers) {
